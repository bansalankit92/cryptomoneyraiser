...
contract CF4{
	...
	function newCampaign(address beneficiary, uint goal, uint durationInMinutes, bytes32  detailsUrl, bytes32  category) public returns (uint campaignID){
        require(beneficiary != 0x0);
	    require(goal>0);
	    campaignID = numCampaigns++; 
	    //deadline = now + durationInMinutes * 1 minutes;
        //can also give reward;
        campaigns[campaignID] = Campaign(beneficiary, goal * 1 ether, now + durationInMinutes * 1 minutes,detailsUrl,category,0, 0);
	}
    ...
    function safeWithdrawal(uint campaignID)isCampaignAvailable(campaignID) public {
        ...
        if (isGoalReached && c.beneficiary == msg.sender) {
